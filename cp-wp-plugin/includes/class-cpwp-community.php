...
final class CPWP_Community {
	...
	public static function publish_from_request() {
		...
		if ( $group ) { update_post_meta( $id, self::GROUP, $group ); update_post_meta( $id, '_cpwp_parent_item', $group ); } CPWP_Moderation::log( 'community_post_created', get_current_user_id(), $id ); return array( '', __( 'Your community post was published.', 'cp-wp-plugin' ) );
	}
