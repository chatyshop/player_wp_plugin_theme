 ...
 * Generic navigation sidebar used by all non-streaming site types.
 */
...
<aside class="cp-sidebar-logged-in">
	<div class="cp-sidebar-user-profile">
		<div class="cp-sidebar-avatar"><?php echo get_avatar( $user->ID, 56 ); ?></div>
	...
	<nav class="cp-sidebar-nav">
		<ul class="cp-sidebar-menu">
			...
			<?php if ( $links ) : ?>
			<li class="cp-sidebar-section-title"><?php echo esc_html( ucfirst( $group ) ); ?></li>
			<?php foreach ( $links as $link ) : ?>
			<li><a href="<?php echo esc_url( $link['url'] ); ?>"><span><?php echo esc_html( $link['label'] ); ?></span></a></li>
			<?php endforeach; ?>
