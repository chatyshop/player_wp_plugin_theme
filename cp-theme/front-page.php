...
<?php endif; ?>
	...
	foreach ( $order as $section ) {
		if ( 'categories' === $section && cp_theme_cp_setting( 'home_show_categories', true ) ) {
			...
			if ( $categories ) { ?><section class="cp-section"><div class="cp-section-head"><h2><?php esc_html_e( 'Explore categories', 'cp-theme' ); ?></h2></div><div class="cp-category-row"><?php foreach ( $categories as $category ) : ?><a class="cp-category-pill" href="<?php echo esc_url( get_category_link( $category ) ); ?>"><?php echo esc_html( $category->name ); ?></a><?php endforeach; ?></div></section><?php }
		}
		...
		if ( 'category_rows' === $section && cp_theme_cp_setting( 'home_show_category_rows', true ) ) {
			foreach ( array_filter( array_map( 'absint', explode( ',', cp_theme_cp_setting( 'home_category_ids', '' ) ) ) ) as $category_id ) {
				$category = get_category( $category_id );
				if ( $category && ! is_wp_error( $category ) ) cp_theme_video_section( $category->name, array( 'cat' => $category_id ), get_category_link( $category_id ) );
			}
		}
